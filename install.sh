...
load_install_settings() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$key" in
            PRINTER_NAME|LDAP_ENABLED|LDAP_HOST|LDAP_PORT|LDAP_USE_SSL|LDAP_BASE_DN|LDAP_BIND_DN|LDAP_BIND_PASSWORD|LDAP_DOMAIN|LDAP_TEST_USER|LDAP_AD_DOMAIN_SID|LDAP_USER_SEARCH_FILTER|LDAP_TLS_REQCERT|SAMBA_ENABLED|SAMBA_REALM|SAMBA_WORKGROUP|SAMBA_HOSTNAME|SAMBA_JOIN_USER|SAMBA_SHARE_NAME|SAMBA_WINDOWS_QUEUE|AIRPRINT_READY_PAPER_SIZES|AIRPRINT_DEFAULT_MEDIA|AIRPRINT_DUPLEX|AIRPRINT_PAPER_MAX|CUPS_USER|CUPS_PASSWORD)
...
# Keep Legal selectable while making A4 the default reported to clients.
AIRPRINT_DEFAULT_MEDIA=iso_a4_210x297mm
