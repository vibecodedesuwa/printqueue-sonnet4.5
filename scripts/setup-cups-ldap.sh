...
load_ldap_settings() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$key" in
            LDAP_HOST|LDAP_PORT|LDAP_USE_SSL|LDAP_BASE_DN|LDAP_BIND_DN|LDAP_BIND_PASSWORD|LDAP_DOMAIN|LDAP_AD_DOMAIN_SID|LDAP_TLS_REQCERT|PRINTER_NAME)
                if [ -z "${!key+x}" ]; then
...
configure_debian_ldap() {
    ...
    package_update
    package_install nslcd libnss-ldapd libpam-ldapd
...
configure_rhel_ldap() {
    ...
    package_update
    package_install openldap-clients sssd sssd-ldap authselect oddjob-mkhomedir
